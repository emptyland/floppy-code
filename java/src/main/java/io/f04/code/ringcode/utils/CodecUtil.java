...
public class CodecUtil {
    ...
    public static short crc16(short crc, byte[] buf) {
        return crc16(crc, buf, 0, buf.length);
    }

    public static short crc16(short crc, byte[] buf, int offset, int size) {
        for (int i = offset; i < size; i++) {
            crc = (short)((crc<<8) ^ CRC16TAB[((crc>>8) ^ buf[i]) & 0x00FF]);
        }
        return crc;
    }
